...
Below are examples showing how you can provide configuration values directly in your prompt *before* including the main content of `INIT_PROMPT.md`. See `INIT_PROMPT.md` for full details on each variable.

```
# Example 1: Keeping demo apps, removing SST only
MONOREPO_ROOT_NAME="demo-eval-project"
REMOVE_DEMO_FULLSTACK="keep_both"
TARGET_PROJECT_GOAL_AND_DESCRIPTION="Learn the monorepo structure and app interaction using the demos, without SST deployment complexity."
# Remove SST config
REMOVE_SST="true"
